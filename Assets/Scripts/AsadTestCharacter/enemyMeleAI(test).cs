...
public class EnemyAI : MonoBehaviour
{
    ...
    public float meleeRange = 1.5f;    // backup attack range if cone fails
    ...
    private void Start()
    {
        ...
        if (damagePlayer == null)
            Debug.LogWarning("⚠️ EnemyAI: DamagePlayer is not assigned in Inspector!");
    }
    ...
    private void Update()
    {
        ...
        if (coneDetection != null)
            Debug.Log("Player in cone? " + coneDetection.PlayerInCone(true));
    ...
    private void Attack()
    {
        ...
        // ✅ Attack only if cooldown ready
        if (attackCooldownTimer <= 0f)
        {
            bool inCone = coneDetection != null && coneDetection.PlayerInCone(true);
            float distanceToPlayer = Vector2.Distance(transform.position, player.position);

            if (inCone || distanceToPlayer <= meleeRange) // backup check
            {
